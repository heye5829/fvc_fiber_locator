...
def run_poly_baseline(image_path, label_path, order=4, max_det_error_px=1.0):
    ...
    # 如果点太少，直接报错更清楚
    if len(src) < 6:
        raise RuntimeError(f"可用基准点过少，无法进行多项式拟合：len(src)={len(src)}")
    ...
    ok_est = tform.estimate(src_norm, dst, order=actual_order)
    if not ok_est:
...
if __name__ == "__main__":
    ...
    for sample_name in sample_names:
        ...
        try:
            res = run_poly_baseline(img_path, lbl_path, order=4, max_det_error_px=1.0)
